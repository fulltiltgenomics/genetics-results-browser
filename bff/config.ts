...
// LD_API_URL          base URL of the FinnGen LD API proxied by GET /api/v1/ld. Proxied rather than
//                     called from the browser because the frontend CSP is `connect-src 'self'`.
...
export const config = {
  ...
  ldApiUrl: (process.env.LD_API_URL ?? "https://api.finngen.fi/api/ld").replace(/\/+$/, ""),
